...
class _DryCleaningPageState extends State<DryCleaningPage> {
  ...
  final double _ratePerKg = 80.0; // Updated rate for dry cleaning per kg in KSh
  ...
  Future<void> _submitBooking() async {
    if (_formKey.currentState!.validate()) {
      try {
        ...
        String orderId = FirebaseFirestore.instance.collection('dry_cleaning_bookings').doc().id;

        // Save booking data to Firestore
        await FirebaseFirestore.instance.collection('dry_cleaning_bookings').doc(orderId).set({
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Dry Cleaning booked! Total: KSh ${_calculatedPrice.toStringAsFixed(2)}')),
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error booking dry cleaning: $e')),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Dry Cleaning Service Booking'),
        backgroundColor: Colors.blueAccent, // Custom color for consistency
      ),
      body: Padding(
  ...
  Widget _buildExpressServiceSwitch() {
    return SwitchListTile(
      ...
      subtitle: const Text('Get your dry cleaning done faster (Extra charges may apply)'),
  ...
  Widget _buildReadyForPickupSwitch() {
    return SwitchListTile(
      ...
      subtitle: const Text('Indicate if the dry cleaning is ready for pickup'),
